...
データは**各端末のブラウザ**にだけ保存されるため、配布物にはデータは含まれません。配布前にクリアする必要はなく、受け取り側は各自のブラウザで初回から空の状態で始まります。  
「データをすべて削除」は、**このブラウザ**に残っている生徒データ・カスタム単元を捨てて、最初からやり直したいとき（テスト後や不要データの整理など）に使います。生徒一覧ページのヘッダー右側にあります。

**初めて開いたとき**  
生徒が1人もいない状態（localStorage に何もないとき）では「デモ生徒」が自動で1人だけ作成されます。そのままクリックして理解地図を試せます。

---

## データの保存場所

**アプリのフォルダ（`dist/` など）の中には保存されません。**

生徒名・理解度・メモなどのデータは、**その PC のブラウザが持っている保存領域（localStorage）** に保存されます。つまり「アプリの外」＝ブラウザ側の領域です。

- **同じ PC・同じブラウザ**で開く限り、データは残ります（アプリのフォルダを別の場所に移しても、そのブラウザで開けばデータは見えます）。
- **別の PC に `dist/` をコピー**した場合、その PC のブラウザにはまだデータがないため、初回はデモ生徒だけの状態になります。
- データはサーバーには送られず、その端末のブラウザ内だけにあります。

---

## 配布について

本プロジェクトは **MIT ライセンス**です。詳細は [LICENSE](LICENSE) を参照してください。
...
- **推奨:** `npx serve dist` を実行し、表示された URL（例: http://localhost:3000）をブラウザで開く。Node が入っていればこれだけで利用できます。
- **公開について:** `npx serve dist` は**あなたの PC 上だけ**で動くローカルサーバーです。インターネットには公開されず、同じ PC のブラウザからだけアクセスできます（デフォルトでは同じ LAN 内の他端末からも見えません）。
- **注意:** `dist/index.html` をブラウザにドラッグして開く方法は**使えません**。本アプリはビルド時に絶対パスで資源を参照するため、`file://` で開くとスクリプトが読み込めず、真っ白な画面になります。必ず `npx serve dist` のようにローカルサーバー経由で開いてください。
...
- **データをすべて削除**  
  このブラウザに保存されている生徒データ・カスタム単元を一括削除（最初からやり直したいとき用）。配布物にはもともとデータは含まれない。
...
- [x] 「データをすべて削除」でブラウザ内の生徒データ・カスタム単元がすべてクリアされる
...
- **保存場所:** アプリのフォルダ内ではなく、**利用しているブラウザの保存領域（localStorage）** にのみ保存。その端末の外には送信しません。
...
**再配布について:** 再配布は禁止しません。ただし、再配布された複製の内容・改変・利用によって生じた問題について、原作者は責任を負いません。再配布は自己の責任で行ってください。再配布する場合は、この README（または LICENSE）の免責条項をそのまま保持することが必要です。
