...
- **何を配布するか**  
  - **パターンA（ソース配布）**: このリポジトリ一式（ZIP または `git clone`）。受け取った側は下記の「使用者の前提」のとおり実行する。  
  - **パターンB（ビルドのみ配布）**: 開発者が `npm run build` した **`dist/` フォルダ一式**を ZIP で渡す。受け取った側は「ビルド済みの開き方」だけ行う。

- **使用者に前提とする環境**  
  - **ソースで渡した場合**: **Node.js**（v18 以上、推奨 v20 以上）と **npm** がインストールされていること。ブラウザは Chrome / Firefox / Safari / Edge など現代のブラウザ。  
  - **ビルドだけ渡した場合**: **ブラウザだけ**あればよい（Node 不要）。`dist/` を簡易サーバーで開く（後述）。

- **Mac でも使えるか**  
  **はい。** Windows / Mac / Linux のいずれでも利用可能です。Node.js は各 OS 用があるためソースの実行も、ブラウザで開くだけの利用も Mac で問題ありません。

- **ビルド済み（`dist/`）の開き方**  
  受け取った人が Node を入れている場合: `npx serve dist` を実行し、表示された URL をブラウザで開く。  
  Node を入れたくない場合: `dist/` 内の `index.html` をブラウザにドラッグして開く方法もあるが、ルーティングや環境によっては不具合が出ることがあるため、**`npx serve dist` を推奨**します。
...
本アプリの再配布は禁止しません。ただし、再配布された複製の内容・改変・利用によって生じた問題について、原作者は責任を負いません。再配布は自己の責任で行ってください。
